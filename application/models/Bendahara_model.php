...
class Bendahara_model extends CI_Model {
    public function get_all_daftar() {
        ...
        // Menghubungkan tabel tb_fakultas dengan left join melalui user.fakultas
        $this->db->join( 'tb_fakultas', 'tb_fakultas.id_fakultas = user.fakultas', 'left' );
        ...
        // Menambahkan kondisi WHERE untuk status = 6
        $this->db->where( 'tb_pendaftaran.status', 6 );
    ...
    public function get_all_user() {
        return $this->db->select( 'user.*, tb_fakultas.nama_fakultas' ) // Memilih kolom yang dibutuhkan
        ->from( 'user' ) // Mengambil data dari tabel user
        ->join( 'tb_fakultas', 'tb_fakultas.id_fakultas = user.fakultas', 'left' ) // Join ke tb_fakultas
        ->where( 'user.level', 'ppkn' ) // Kondisi level = 'ppkn'
        ->get()
        ->result();
    }
